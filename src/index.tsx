...
Доделать:
  ...
  10. 7-2-3 Для обработки статуса 401 можно воспользоваться механизмом перехватчиков в axios.
  11. одинаковый код FavoriteItem и LoginPage обработка клика на название города
    параметр только название города
  12. OfferPage при загрузке данных отрисовывает не надолго 404
  ...
  13. const offerReviews = [...reviews] // ... т.к. при ассинхронном действии успевает затираться
    может что то будет для 17
  14. //!! очиска формы evt.target.reset
  15. блокировка формы отправки комментария
  16. При логине/логауте обновлять списка предложений, т.к. там появляються и исчезают отметки избранное
    сделал
    ...
    ! так была ошибка при реакта при отрисовке, либо в начале, если поментять очередность загрузки то при входе выходе
    ! поставил загрузку предложений после проверки статуса авторизации и после выхода и входа
  17. нужно отследить проверку успешности логина пароля
    блокировать форму на время запроса
    не сбрасывать форму и не переходить на главную
    в хранилище добавить ко все данным статус запроса
  18. нужно отследить проверку успешности отправки комментария
    блокировать форму на время запроса
    не сбрасывать форму
    в хранилище добавить ко все данным статус запроса
